...
/*
 * sx1276 context related
 */
static sx1276_context dev;
static m_settings settings;
static radioLoRaSettings_t loraSettings;
//static radioLoRaPacketHandler_t _dev_lora_packet_handler;
...
struct k_timer MacStateCheckTimer;
 ...
 */
static RadioEvents_Callbacks RadioEvents;

/*!
 * LoRaMac duty cycle delayed Tx timer
 */
//static TimerEvent_t TxDelayedTimer;
struct k_timer TxDelayedTimer;

/*!
 * LoRaMac reception windows timers
 */
// using k timers here for Zephyr
struct k_timer RxWindowTimer1;
struct k_timer RxWindowTimer2;
...
struct k_timer AckTimeoutTimer;
...
/*
 * Implementation of a reverse memcpy function based on an implementation
 * provided for the stm32.
 */
                ...
                if(isMicOk == true) {
                    ...
                    if(skipIndication == false) {
                        ...
                        if(fCtrl.Bits.Ack == 1) {
                            ...
                            // are needed.
                            // zephyr based k_timer used here
...
static bool RxWindowSetup( uint32_t freq, int8_t datarate, uint32_t bandwidth, uint16_t timeout, bool rxContinuous )
{
    ...
    //if( Radio.GetStatus( ) == RF_IDLE )
        ...
        if( datarate == DR_7 )
        {
            modem = MODEM_FSK;
            // add sx1276 call here
            sx1276_set_rx_config(dev, modem, 50e3, downlinkDatarate * 1e3, 0, 83.333e3, 5, 0, false, 0, true, 0, 0, false, rxContinuous);
        ...
        {
            // add sx1276 call here
            // this should be the set_rx call, need to confirm
...
LoRaMacStatus_t SendFrameOnChannel( ChannelParams_t channel )
{
    ...
    { // High Speed FSK channel
        // add sx1276 calls here
        sx1276_set_max_payload_length(dev, MODEM_FSK, LoRaMacBufferPktLen);
        sx1276_set_tx_config(dev, MODEM_FSK, txPower, 25e3, 0, datarate * 1e3, 0, 5, false, true, 0, 0, false);
        TxTimeOnAir = sx1276_get_time_on_air(dev, MODEM_FSK, LoRaMacBufferPktLen);
    ...
    { // High speed LoRa channel
        ...
        sx1276_set_tx_config(dev, MODEM_LORA, txPower, 0, 1, datarate, 1, 8, false, true, 0, 0, false);
...
LoRaMacStatus_t LoRaMacChannelAdd( uint8_t id, ChannelParams_t params )
{
    ...
    // add sx1276 call here
    if((sx1276_check_rf_frequency(dev, params.Frequency) == true) && (params.Frequency > 0) && (frequencyInvalid == false))
